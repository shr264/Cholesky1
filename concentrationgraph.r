concentrationgraph <- function(Y,adjctMat,order,tol,standard){
time1 = proc.time()
...
if(dim(E1)[1]==0){
    ...
    maxCsize = qpCliqueNumber(filledadjacencyMatrix)
    if(maxCsize>n){
        cat('Largest Clique size is larger than n. Algorithm will probably fail!\n')}
    cat('Calculating Ld\n')
...
filledadjacencyMatrix = filledadjacencyMatrix[reverseordering,reverseordering]
time2 = proc.time()
